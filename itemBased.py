...
def predictRating(recommend_data, toBeRatedPath):

	M, sim_item = crossValidation(recommend_data)

	#f = open("toBeRated.csv","r")
	f = open(toBeRatedPath,"r")
	toBeRated = {"user":[], "item":[]}
	for row in f:
		r = row.split(',')	
...
trainingRatingPath = 'item-based-in/ratings.base'
recommend_data = readingFile(trainingRatingPath)
#crossValidation(recommend_data)
predictRating(recommend_data, 'item-based-in/toBeRated.test')